...
export function softZoneCheck(lat, lon) {
  ...
  if (km > 30) {
    warn.textContent = (typeof t === 'function' ? t('checkout.zoneWarning', 'Note: Your address is approximately {distance} km from Nashik. Delivery availability may vary.') : 'Note: Your address is approximately {distance} km from Nashik.').replace('{distance}', km.toFixed(1));
...
export function initPincodeValidation() {
...
export function updateItemUI(item) {
...
export function handleQuantityChange(itemId, change) {
...
export function placeOrderRazorpay() {
  const finalTotalEl = document.getElementById("finalTotal");
...
// Preserve global window bindings for backward compatibility
